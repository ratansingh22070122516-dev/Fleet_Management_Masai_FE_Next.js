  ...
  const handleSubmit = async (values: any, { setSubmitting }: any) => {
    try {
      const payload = {
        ...
        // Keep existing images, don't send blob URLs
        images: vehicle?.images || []
      }

      const response = await vehicleApi.update(vehicleId, payload)
            ...
            <div className="card">
              <Formik
                initialValues={{
                  ...
                  passengers: vehicle.capacity?.passengers || 5,
                  cargo: vehicle.capacity?.cargo || 10,
                  baseRate: vehicle.pricing?.baseRate || 100,
                  rateType: vehicle.pricing?.rateType || 'daily',
                  street: vehicle.location?.address || '',
